...
namespace Cupscale
{
	class Config
	{
		...
		{
			try {
				if(settingsForm != null){ settingsForm.Enabled = false; }
				...
				if(settingsForm != null){ settingsForm.Enabled = true; }
			} catch(Exception e){
				retries -= 1;
		...
		public static string Get(string key, Type type = Type.String)
		{
			...
			try {
				if(cachedValues.ContainsKey(keyStr)){ return cachedValues[keyStr]; }
		...
		{
			return Get(key, Type.UInt).GetUInt();
		}
		...
		public static uint GetUInt(string key, uint defaultVal)
		{
			WriteIfDoesntExist(key.ToString(), defaultVal.ToString());
			return GetUInt(key);
		}
		...
		{
			if(cachedValues.ContainsKey(key.ToString())){ return; }
			Set(key, val);
		}

		public enum Type { String, Int, UInt, Float, Bool }
		private static string WriteDefaultValIfExists(string key, Type type)
		{
			...
			if(type == Type.Int || type == Type.UInt || type == Type.Float){ return WriteDefault(key, "0"); }     // Write default int/float (0)
