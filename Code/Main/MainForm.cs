...
namespace Cupscale.Main
{
	public partial class MainForm : Form
	{
		...
		async Task CheckDependenciesAsync()
		{
			...
			bool hasAnyPy = (await Task.Run(Dependencies.SysPyAvail) || await Task.Run(Dependencies.EmbedPyAvail));
			bool hasNvGpu = NvApi.gpuList.Count > 0; 

			if(!hasAnyPy && hasNvGpu){
				DialogResult dialog = MessageBox.Show(
					"You have no Python runtime installed, which is required for CUDA-based upscaling! " +
		...
		{
			//// https://github.com/microsoft/onnxruntime/issues/8995
			//// https://github.com/microsoft/onnxruntime/issues/6215
			/// REMIDNER SET VRAM CUDA LIMITS
			vramLabel.Text = text;
		...
		{
			if(!string.IsNullOrWhiteSpace(s)){ upscaleBtn.Text = s; }
		}
		...
		private async void upscaleBtn_Click(object sender, EventArgs e)
		{
			...
			if(!PreviewUi.HasValidModelSelection()){ return; }

			if(Config.GetBool("reloadImageBeforeUpscale")){ ReloadImage(); }
		...
		private async void refreshPreviewFullBtn_Click(object sender, EventArgs e)
		{
			if(Config.GetBool("reloadImageBeforeUpscale")){ ReloadImage(); }
			UpdateResizeMode();
			await Task.Run(() => PreviewUi.UpscalePreview(true));
		}
		...
		private async void MainForm_KeyUp(object sender, KeyEventArgs e)
		{
			if(e.KeyData == (Keys.Control | Keys.V)){
				if(htTabControl.SelectedIndex != 0 || !Enabled){ return; }
				try {
		...
		private void previewImg_Click(object sender, EventArgs e)
		{
			...
			if(mouseEventArgs.Button == MouseButtons.Right){
				...
				if(fileDialog.ShowDialog() == CommonFileDialogResult.Ok)
					{ _ = LoadImages(fileDialog.FileNames.ToArray()); }
			}
		}
