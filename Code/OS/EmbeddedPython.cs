...
namespace Cupscale.OS
{
	class EmbeddedPython
	{
		...
		static async Task Install()
		{
			...
			if(Directory.Exists(extractPath)){ IoUtils.ClearDir(extractPath); }
			...
			MsgBox msg = Program.ShowMessage(
				"The Python files will now be compressed to reduce the amount of storage space needed " +
			...
			await Init();
			MsgBox msg2 = Program.ShowMessage(
				"Installed embedded Python runtime and enabled it!\nIf you want to disable it, you can do so in the settings.", "Message"
			);
