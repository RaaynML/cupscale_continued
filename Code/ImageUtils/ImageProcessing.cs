	...
	internal class ImageProcessing
	{
		...
		public static async Task ConvertImage(string path, Format format, bool fillAlpha, ExtMode extMode, bool deleteSource = true, string overrideOutPath = "", bool allowTgaFlip = false)
		{
			...
			if(format == Format.Weppy){
				...
				if(img.Quality >= 100){ img.Settings.SetDefine(MagickFormat.WebP, "lossless", true); }
				newExt = "webp";
			}
			...
			if(magick){
				img = CheckColorDepth(path, img);
				if(fillAlpha){ img = ImgUtils.FillAlphaWithBgColor(img); }
			}
		...
		{
			if(!string.IsNullOrWhiteSpace(overrideOutPath)){ return overrideOutPath; }
			if(extMode == ExtMode.UseNew){ return Path.ChangeExtension(path, newExt); }
			if(extMode == ExtMode.KeepOld){ return path; }
			if(extMode == ExtMode.AppendNew){ return path + "." + newExt; }
		...
		public static async Task PostProcessImage(string path, Format format, bool dontResize)
		{
			...
			if(!dontResize){ ResizeImagePost(img); }
