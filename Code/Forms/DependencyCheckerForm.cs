...
namespace Cupscale.Forms
{
	public partial class DependencyCheckerForm : Form
	{
		...
		{
			InitializeComponent();
			if(openPyInstaller){ tabList1.SelectedIndex = 1; }
			if(openPyInstaller && startPyInstall){ installBtn_Click(null, null); }
		}
		...
		public async new Task Refresh()
		{
			...
			if(!sysPyAvail && embedPyAvail){ SetGrey(sysPython, "Not Needed"); }

			if(!embedPyAvail && sysPyAvail){ SetGrey(embedPython, "Not Needed"); }
		...
		{
			if(!installBtn.Enabled){ e.Cancel = true; }
		}
	}
}
